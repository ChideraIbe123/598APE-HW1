        ...
        while (lscanf(f, "%s", object_type) != EOF) {
            if (streq(object_type, "light")) {
                ...
                if (lscanf(f, "%lf %lf %lf %d %d %d\n", &light_x, &light_y, &light_z, &color_r,
                           &color_g, &color_b) == EOF) {
                    printf("Could not read <light_x> <light_y> <light_z> <color_r> <color_g> "
                           "<color_b>\n");
                    exit(1);
                }
                ...
                if (lscanf(f, "%lf %lf %lf %lf %lf %lf %lf %lf\n", &plane_x, &plane_y, &plane_z,
                    ...
                    printf("Could not read <plane_x> <plane_y> <plane_z> <yaw> <pitch> <roll> <tx> "
                           "<ty>\n");
                    exit(1);
                }
                Texture* texture = parseTexture(f, false);
                Plane* shape =
                    new Plane(Vector(plane_x, plane_y, plane_z), texture, yaw, pitch, roll, tx, ty);
                ...
                if (lscanf(f, "%lf %lf %lf %lf %lf %lf %lf %lf\n", &disk_x, &disk_y, &disk_z, &yaw,
                    ...
                    printf("Could not read <disk_x> <disk_y> <disk_z> <yaw> <pitch> <roll> <tx> "
                           "<ty>\n");
                    exit(1);
                }
                Texture* texture = parseTexture(f, false);
                Disk* shape =
                    new Disk(Vector(disk_x, disk_y, disk_z), texture, yaw, pitch, roll, tx, ty);
                MAIN_DATA->addShape(shape);
                shape->normalMap = parseTexture(f, true);
                ...
                if (lscanf(f, "%lf %lf %lf %lf %lf %lf %lf %lf\n", &box_x, &box_y, &box_z, &yaw,
                           &pitch, &roll, &tx, &ty) == EOF) {
                    printf(
                        "Could not read <box_x> <box_y> <box_z> <yaw> <pitch> <roll> <tx> <ty>\n");
                    exit(1);
                }
                Texture* texture = parseTexture(f, false);
                Box* shape =
                    new Box(Vector(box_x, box_y, box_z), texture, yaw, pitch, roll, tx, ty);
                MAIN_DATA->addShape(shape);
                shape->normalMap = parseTexture(f, true);
                ...
                if (lscanf(f, "%lf %lf %lf %lf %lf %lf %lf\n", &sphere_x, &sphere_y, &sphere_z,
                    ...
                    printf("Could not read <sphere_x> <sphere_y> <sphere_z> <yaw> <pitch> <roll> "
                           "<radius>\n");
                    exit(1);
                }
                Texture* texture = parseTexture(f, false);
                Sphere* shape = new Sphere(Vector(sphere_x, sphere_y, sphere_z), texture, yaw,
                                           pitch, roll, radius);
                MAIN_DATA->addShape(shape);
                shape->normalMap = parseTexture(f, true);
                ...
                if (lscanf(f, "%s %d %s %d %lf %lf %lf\n", point_filepath, &num_points,
                    ...
                    printf("Could not read <point filepath> <num_points> <polygons filepath> "
        ...
        while (lscanf(f, "%s %s %d %s %lf %lf", transition_type, object_type, &obj_num, field_type,
            ...
            if (streq(object_type, "camera")) {
                if (streq(field_type, "yaw")) {
                    ...
                    printf(
                        "Unknown camera field_type %s, expected one of yaw, pitch, roll, x, y, z\n",
                        field_type);
                    exit(1);
                }
...
int main(int argc, const char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (streq(argv[i], "--help")) {
            printf("Usage %s [-H <height>] [-W <width>] [-F <framecount>] [--movie] [--no-movie] "
    ...
    if (frameLen > 1 && toMovie) {
        if (png) {
            snprintf(command, sizeof(command),
                     "ffmpeg -y -r 24 -i %s.tmp.%%07d.png -vcodec ffv1 %s.tmp.avi && ffmpeg -y -i "
                     "%s.tmp.avi -c:v libx264 -preset veryslow -qp 0 -r 24 %s",
                     outFile, outFile, outFile, outFile);
        } else {
            snprintf(command, sizeof(command),
                     "ffmpeg -y -r 24 -i %s.tmp.%%07d.ppm -vcodec ffv1 %s.tmp.avi && ffmpeg -y -i "
                     "%s.tmp.avi -c:v libx264 -preset veryslow -qp 0 -r 24 %s",
                     outFile, outFile, outFile, outFile);
        }
